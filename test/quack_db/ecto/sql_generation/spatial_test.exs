defmodule QuackDB.Ecto.SQLGeneration.SpatialTest do
  ...
  test "generates spatial fragments" do
    query =
      from(place in "places",
        where:
          intersects(place.geom, ^%Geo.Point{coordinates: {1.0, 2.0}, srid: nil}) and
            st_contains(envelope(0, 0, 10, 10), place.geom),
        select: %{
          point: point(1, 2),
    ...
    assert query |> Ecto.Adapters.QuackDB.Connection.all() |> IO.iodata_to_binary() ==
             ~s|SELECT ST_Point(1, 2) AS "point", ST_AsWKB(q0."geom") AS "wkb", ST_AsHEXWKB(q0."geom") AS "hexwkb", ST_AsText(q0."geom") AS "wkt", ST_AsGeoJSON(q0."geom") AS "geojson", ST_Distance(q0."geom", ?) AS "distance" FROM "places" AS q0 WHERE (ST_Intersects(q0."geom", ?) AND ST_Contains(ST_MakeEnvelope(0, 0, 10, 10), q0."geom"))|
  end
end
