  ...
  test "generated temporal and decimal defaults run against DuckDB" do
    ...
    assert %{
             ...
             QuackDB.IntegrationRepo.query!(
               "SELECT id, amount, event_date, event_time, occurred_at, precise_at, received_at, precise_received_at FROM #{table}"
             )
    ...
    assert NaiveDateTime.compare(precise_at, ~N[2026-05-26 12:34:56.123456]) == :eq
    assert DateTime.compare(received_at, ~U[2026-05-26 12:34:56Z]) == :eq
    assert DateTime.compare(precise_received_at, ~U[2026-05-26 12:34:56.123456Z]) == :eq
