...
- Added `Duration.t()` SQL parameter support, Ecto `time_bucket/2,3` compatibility for pinned Elixir durations and origins, typed Ecto `series/1,3` helpers for DuckDB `generate_series` date/timestamp sources, JSON SQL parameters/path helpers, source sampling helpers, enumerable stream append, and Table.Reader input append.
